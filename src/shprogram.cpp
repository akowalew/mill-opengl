...
GLuint compile_vertex_shader(const GLchar* shaderCode)
{
	std::cout << "Vertex shader: " << shaderCode << '\n';
...
GLuint compile_fragment_shader(const GLchar* shaderCode)
{
	std::cout << "Fragment shader: " << shaderCode << '\n';
...
ShaderProgram::ShaderProgram(const GLchar* vertexPath, const GLchar* fragmentPath)
{
	...
	GLuint vertex_id = compile_vertex_shader(vertex_code.c_str());

	string fragment_code = read_shader_code(fragmentPath);
	GLuint fragment_id = compile_fragment_shader(fragment_code.c_str());
